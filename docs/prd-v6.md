...
|---|---|---|---|
| Areas (Regions, Countries, Cities) | — (internal only) | Global Sys Admin or Service Desk manage the full tree | **New in v6** — was "Regions" (flat, Sys-Admin-only) in v5 |
| Sites, Buildings, Rooms | Global Admin sees roll-up across enrolled sites; Site Contact sees their one site | Global Sys Admin or Service Desk manage; Room is new | Room level new in v6 |
...
**Delegated master data management:** a Service Desk agent opens Areas, adds a new Country under an existing Region, adds a City under it, then creates the Site, its Buildings and Rooms — all without a Sys Admin. Every step revalidates the same page, so the hierarchy is visible immediately.
