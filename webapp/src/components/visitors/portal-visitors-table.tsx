...
import type { Building, Facility, SiteEnrollment, Visitor, VisitorRequest } from "@prisma/client";
...
export function PortalVisitorsTable({ requests }: { requests: RequestRow[] }) {
  const columns: DataTableColumn<RequestRow>[] = [
    {
      ...
      searchValue: (r) => `${r.purpose} ${r.building?.name ?? ""}`,
    },
    {
      key: "site",
      header: "Site",
    ...
    { key: "source", header: "Source", cell: (r) => r.source, filterOptions: uniqueOptions(requests.map((r) => r.source)), filterValue: (r) => r.source },
    {
      key: "status",
      header: "Status",
    ...
    { key: "acsSync", header: "ACS sync", cell: (r) => <StatusBadge status={r.acsSyncStatus} />, sortValue: (r) => r.acsSyncStatus },
  ];

  return (
    <DataTable columns={columns} rows={requests} getRowKey={(r) => r.id} searchPlaceholder="Search visitor requests…" emptyMessage="No visitor requests yet." />
  );
}
