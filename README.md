...
- **`docs/`** — the product requirements documents and business plan, in order. `prd-v6.md` is current; v1–v5 are kept for history, each with a pointer to what superseded it. `product-phase-summary.md` is the running status snapshot. `installation-guide-ubuntu.md` is the newbie install walkthrough for `webapp/`.
