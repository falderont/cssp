...
import { getFacilityTabAccess, firstFacilityTabPath } from "@/lib/facility-tabs";
...
export default async function FacilityOverviewPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const user = await requireFacilityPageAccess();
  const { isMasterDataAdmin } = getFacilityTabAccess(user.role);
  const isSysAdmin = user.role === ROLES.SYS_ADMIN;

  // Every other role that can reach this site's page at all lands on a tab
  // it can actually use — Overview is master-data-admin territory
  // (buildings, rooms, site settings, danger zone).
  if (!isMasterDataAdmin) redirect(firstFacilityTabPath(id, user.role));
