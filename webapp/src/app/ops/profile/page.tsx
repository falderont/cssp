...
export default async function OpsProfilePage() {
  const sessionUser = await requireInternalUser();
  const user = await prisma.user.findUniqueOrThrow({ where: { id: sessionUser.id } });

  return (
    <div>
      <PageHeader title="Profile & preferences" description="Your personal account details and how the console looks for you." />
        ...
        <Card className="lg:col-span-1">
          ...
          <CardBody className="space-y-4">
            ...
            <dl className="space-y-2 text-sm">
              ...
              {user.csScope && (
                <div className="flex justify-between gap-3">
                  <dt className="text-slate-500">CS scope</dt>
                  <dd className="font-medium text-slate-800">{user.csScope}</dd>
                </div>
              )}
              {user.title && (
                <div className="flex justify-between gap-3">
            ...
            <p className="text-xs text-slate-400">To change your name, email or role, contact your Global Sys Admin.</p>
          </CardBody>
        </Card>
