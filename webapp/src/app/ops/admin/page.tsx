...
export default async function AdminIndexPage() {
  ...
  const [regions, teams, accounts, userCount, blacklistCount, pendingAal] = await Promise.all([
    prisma.region.findMany({
      include: {
        countries: {
          include: {
            facilities: { include: { buildings: true, siteEnrollments: true } },
          },
        },
      },
      orderBy: { name: "asc" },
    ...
    prisma.enterpriseAccount.findMany({
      include: { siteEnrollments: { include: { facility: { include: { country: { include: { region: true } } } } } } },
    }),
    prisma.user.count(),
  ...
  const facilityCount = regions.reduce((sum, r) => sum + r.countries.reduce((s, c) => s + c.facilities.length, 0), 0);
  const buildingCount = regions.reduce(
    (sum, r) => sum + r.countries.reduce((s, c) => s + c.facilities.reduce((s2, f) => s2 + f.buildings.length, 0), 0),
    0
  );
  ...
  const footprint = accounts
    .map((a) => {
      ...
      const countries = new Set(a.siteEnrollments.map((e) => e.facility.countryId));
      const regionIds = new Set(a.siteEnrollments.map((e) => e.facility.country.regionId));
  ...
  const quickLinks = [
    ...
    },
    {
      ...
      items: [
        { href: "/ops/admin/regions", icon: Map, title: "Regions", description: `${regions.length} region(s) — top-level, multi-country groupings.` },
        { href: "/ops/admin/countries", icon: Globe2, title: "Countries", description: `${countryCount} country(ies) across every region.` },
        { href: "/ops/admin/facilities", icon: Building2, title: "Facilities & buildings", description: `${facilityCount} facility(ies), ${buildingCount} building(s).` },
        { href: "/ops/admin/accounts", icon: Building2, title: "Tenant management", description: `${accounts.length} tenant account(s) — master data & site enrollments.` },
      ],
    },
    {
      section: "Platform",
  ...
  return (
    <div>
      <PageHeader
        ...
        description="The Global Sys Admin's control tower — every region, country, site, tenant and team, at a glance. Master data lives here first; sites and tenants are provisioned against it."
      />
        ...
        <Card>
          ...
          <Table>
            ...
            <TBody>
              ...
              {regions.map((r) => {
                const regionFacilities = r.countries.reduce((s, c) => s + c.facilities.length, 0);
                const regionBuildings = r.countries.reduce((s, c) => s + c.facilities.reduce((s2, f) => s2 + f.buildings.length, 0), 0);
                const regionTenants = new Set(r.countries.flatMap((c) => c.facilities.flatMap((f) => f.siteEnrollments.map((e) => e.enterpriseAccountId))));
                return (
                  <Fragment key={r.id}>
                    ...
                    {r.countries.map((c) => {
                      const tenants = new Set(c.facilities.flatMap((f) => f.siteEnrollments.map((e) => e.enterpriseAccountId)));
                      const buildings = c.facilities.reduce((s, f) => s + f.buildings.length, 0);
                      return (
                        <TR key={c.id}>
